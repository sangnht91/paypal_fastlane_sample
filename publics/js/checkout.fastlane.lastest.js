...
const PaypalSdk = {
  ...
  initPaypalSdk: async() => {
    ...
    const { clientToken, clientId } = await PaypalSdk.getMid()
    var script = document.createElement('script')
    script.type = 'text/javascript'
    script.setAttribute('data-sdk-client-token', clientToken)
    script.src = PaypalSdk.getPaypalSDK(clientId)
    script.onload = () => {
      CheckoutFastLane()
    }
...
const CheckoutFastLane = async () => {
  ...
  const placeOrder = () => {
    FastlaneElements.elmFastlaneSB && FastlaneElements.elmFastlaneSB.addEventListener('click', async() => {
      ...
      const payload = {
        productName: "",
        productPrice: ctrwowCheckout?.checkoutData?.getProduct()?.productPrices?.DiscountedPrice?.Value || 0,
        couponCode: null,
        ...
        customer: {
          ...
          phoneNumber: _shippingAddress.phoneNumber.nationalNumber,
          firstName: _shippingAddress.name.firstName,
          lastName: _shippingAddress.name.lastName,
        },
        payment: {
          paymentProcessorId: 1000,
          paymentToken: _paymentToken
        },
        shippingAddress: {
          firstName: _shippingAddress.name.firstName,
          lastName: _shippingAddress.name.lastName,
          address1: _shippingAddress.address.addressLine1,
          address2: _shippingAddress.address.addressLine2,
          city: _shippingAddress.address.adminArea2,
          countryCode: _shippingAddress.address.countryCode,
          state: _shippingAddress.address.adminArea1,
          zipCode: _shippingAddress.address.postalCode,
          phoneNumber: _shippingAddress.phoneNumber.nationalNumber
        },
        billingAddress: null,
        funnelBoxId: 0
